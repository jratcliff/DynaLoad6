...
/**
 * @class Ext.ux.PackageLoader4
 * @singleton
 * @version 4.0
 * @author Doug Hendricks, Sencha Services
 * @platform Ext 4.2+, Sencha Command 4.0.4+ REQUIRED
 *
 * A dynamic Sencha Command Package Loader for ExtJs 4.2
 *
 *   <h2>Preface:</h2>
 *
 ...
 *  @example
    ...
    // create YOUR OWN arbitrary properties to suite application needs
        "defaultViewClass" : "Fanciful.view.Ledger"  // <- designate the default View for rendering
     }
 *
 ...
 *  @example
    ...
    Ext.require( 'Ext.ux.PackageLoader4', function() {
 ...
 Ext.define('Fanciful.controller.Navigation', {
     ...
     onModuleAction : function(item) {
        ...
        Ext.requirePackage( item.moduleAction, function callback(success, results) {
             ...
             if(success && config.defaultViewClass) // custom 'defaultViewClass' property
              {
                  this.child('component[region=center]').add(
                     Ext.create( config.defaultViewClass)
                  );

              } else {
                 //otherwise, config is simply the failed string package name
                  Ext.log.warn('Package ' + config + ' was not available or, could not determine the view to render: ' + config.defaultViewClass);
              }
              item.enable();
 ...
 * @example
     "requires": [
      ...
      "defaultViewClass" : "Fanciful.view.Ledger",
...
Ext.define('Ext.ux.PackageLoader4', {
    ...
    gatherPackageAssets :function(packageName, packageEntry) {
        ...
        var me = Ext.ux.PackageLoader,
            ...
            onLoad = function() {
                if (urls) {
                    url = urls.pop();
                    if (!urls.length && url) {   // 'twas the last one?
                        me.onPackageLoaded(name);
                        url = urls = null;
                    }
                }
            },
            ...
            onError = function() {
                url = urls = null;    //abort further onLoad handling after first error
                me.onPackageError(name);
            };
        ...
        if (config.skipResources !== true) {
            ...
            url = packagePath + 'resources/' + name +
                (me.enableRtl ? '-all-rtl' : '-all') +
                (me.enableDebug ? '-debug.css' : '.css');

            if ( me.disableCaching ) {
                url += '?_dc=' + now;
            }

            /**
             * Sadly, < Safari 5 does not support load/error events on <link> tags, so optimism is required here
             * (ie. no urls.length tracking [above] for CSS)
             */
            Ext.isSafari5 || urls.push(url);

            (doc.head || doc.getElementsByTagName('head')[0]).appendChild(
                  Ext.apply(doc.createElement('link'), {
                      rel       : 'stylesheet',
                      href      : url,
                      onreadystatechange : me._hasLegacyReadyState ? readyStateChange : null,
                      onload    : me._hasLegacyReadyState ? Ext.emptyFn : onLoad,
                      onerror   : onError
                  })
            );
        }
        ...
        if (config.skipJS !== true ) {
            ...
            url = packagePath + name + (me.enableDebug ? '-debug.js' : '.js');
        ...
        if (!url) {
            // <debug>
            Ext.log.error('Package '+ name + ' is not configured to load anything.');
            // </debug>
            me.onPackageError(name);
        }
    },
     ...
     * @param {String} packageName
     */
    onPackageError : function(packageName) {
        // <debug>
        Ext.log.error('package load failed: ' + packageName + ' (Is it in the package build path yet?)');
        // </debug>
    ...
    assertPackage : function(packageName) {
        var me = Ext.ux.PackageLoader,
            ...
            manifest = Ext.manifest || (Ext.manifest = {requires : [], packages : {}});
        ...
        if(!thePackage.loaded &&
            ( Ext.Array.contains(manifest.requires, packageName) ||
                packageName in manifest.packages)
            ) {
    ...
    var pLoader = Ext.ux.PackageLoader = this,
    ...
    /**
     * @private
     * @property {boolean} _hasLegacyReadyState
     * Assert =< IE8/9+ overlap in DOM Level 2 event coverage.  If addEventListener is a valid method
     * for the Script Element, it's 'onload' event is preferred over 'onreadystatechange'
     */
    pLoader._hasLegacyReadyState = !('addEventListener' in script) && ('readyState' in script);
    ...
    Ext.create('Ext.data.Connection').request({
        ...
        callback    : function(options, success, response ) {
            ...
            else if (config.requires) {
                manifest.requires = manifest.requires.concat(config.requires);
            }
        }
    });
    // </debug>
