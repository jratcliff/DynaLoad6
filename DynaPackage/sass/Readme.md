...
folder are mapped to the application's root namespace, 'DynaPackage'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "DynaPackage"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'DynaPackage' in the same way
as `"DynaPackage/sass/src"`.
